...
def test_letter_matching():
    ...
    # Test i/ı equivalence
    assert validator.letters_match('i', 'ı'), "i should match ı"
    assert validator.letters_match('ı', 'i'), "ı should match i"
    assert validator.letters_match('i', 'i'), "i should match i"
    assert validator.letters_match('ı', 'ı'), "ı should match ı"
    print("✓ i/ı equivalence works")
...
def test_all_equivalence_pairs():
    """Test all 6 equivalence pairs in one word."""
    print("\nTesting all equivalence pairs together...")

    # Test with a word containing all equivalences: 'çığöüş'
    target = "cigous"  # c, i, g, o, u, s
    guess = "çığöüş"   # ç, ı, ğ, ö, ü, ş
    result = validator.validate_guess(guess, target)
    ...
    assert result[1]['letter'] == 'i', "Position 1 should display 'i'"
    assert result[2]['letter'] == 'g', "Position 2 should display 'g'"
    assert result[3]['letter'] == 'o', "Position 3 should display 'o'"
    assert result[4]['letter'] == 'u', "Position 4 should display 'u'"
    assert result[5]['letter'] == 's', "Position 5 should display 's'"
    print("✓ All 6 equivalence pairs work together")
